  ...
  async function loadProfiles() {
    try {
      ...
      if (filters.location && filters.radius) {
        console.log('Applying location filter:', { location: filters.location, radius: filters.radius });
        
        const radius = parseInt(filters.radius);
        const distancePromises = filteredProfiles
          .filter(profile => profile.location) // Only check profiles with location
          .map(async profile => {
            try {
              const distance = await calculateDistance(filters.location, profile.location!);
        ...
        console.log('Distance results:', distanceResults.map(r => ({ 
          profile: r.profile.username, 
          distance: r.distance 
        })));
        
        filteredProfiles = distanceResults
      ...
      // Apply skill filter
      if (filters.skill) {
        console.log('Applying skill filter:', filters.skill);
      ...
      console.log('Final filtered profiles count:', filteredProfiles.length);
