...
export async function checkUsernameAvailability(username: string): Promise<boolean> {
  try {
    // Instead of using .single(), which expects exactly one row,
    // use standard select which returns an array
    const { data, error } = await supabase
      .from('profiles')
      .select('username')
    ...
    // If data is null or an empty array, the username is available
